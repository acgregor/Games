...
public class DragonCurve extends JPanel implements KeyListener
{
	...
	{
		if(e.getKeyCode()==KeyEvent.VK_LEFT && colorChange>2)
		{
			colorChange/=2;
			repaint();
		}
		if(e.getKeyCode()==KeyEvent.VK_RIGHT && colorChange<=2048)
		{
			colorChange*=2;
			repaint();
		}
		if(e.getKeyCode()==KeyEvent.VK_R || e.getKeyCode()==KeyEvent.VK_UP || e.getKeyCode()==KeyEvent.VK_DOWN)
		{
			if(e.getKeyCode()==KeyEvent.VK_UP)
			{
				length++;
			}
			else if(e.getKeyCode()==KeyEvent.VK_DOWN && length>1)
			{
				length--;
			}
		...
		if(e.getKeyCode()==KeyEvent.VK_SPACE && iteration<20)
		{
			int size=c1.size();
